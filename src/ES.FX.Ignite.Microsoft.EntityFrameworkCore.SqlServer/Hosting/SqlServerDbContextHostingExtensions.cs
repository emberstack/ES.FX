...
public static class SqlServerDbContextHostingExtensions
{
    ...
    /// <param name="configureOptions">An optional delegate that can be used for customizing options. It's invoked after the options are read from the configuration.</param>
    /// <param name="configureDbContextOptionsBuilder">An optional delegate that can be used for customizing the <see cref="DbContextOptionsBuilder"/>.</param>
    /// <param name="configureSqlServerDbContextOptionsBuilder">An optional delegate that can be used for customizing the <see cref="SqlServerDbContextOptionsBuilder"/>.</param>
    /// <param name="lifetime">The lifetime of the <see cref="DbContext" />. Default is <see cref="ServiceLifetime.Transient" />.</param>
    /// <param name="configurationSectionKey">The configuration section key. Default is <see cref="DbContextSpark.ConfigurationSectionKey" />.</param>
    ...
    /// <summary>
    /// Registers a <see cref="IDbContextFactory{TDbContext}"/> as a service in the services provided by the <paramref name="builder"/>.
    /// Enables retries, health check, logging and telemetry for the <see cref="DbContext" />.
    /// </summary>
    /// <typeparam name="TDbContext">The <see cref="TDbContext" /> type used by the <see cref="IDbContextFactory{TDbContext}"/> that needs to be registered.</typeparam>
    /// <param name="builder">The <see cref="IHostApplicationBuilder" /> to read config from and add services to.</param>
    /// <param name="key">A name used to retrieve the settings and options from configuration</param>
    /// <param name="configureSettings">An optional delegate that can be used for customizing settings. It's invoked after the settings are read from the configuration.</param>
    /// <param name="configureOptions">An optional delegate that can be used for customizing options. It's invoked after the options are read from the configuration.</param>
    /// <param name="configureDbContextOptionsBuilder">An optional delegate that can be used for customizing the <see cref="DbContextOptionsBuilder"/>.</param>
    /// <param name="configureSqlServerDbContextOptionsBuilder">An optional delegate that can be used for customizing the <see cref="SqlServerDbContextOptionsBuilder"/>.</param>
    /// <param name="lifetime">The lifetime of the <see cref="IDbContextFactory{TDbContext}"/>. Default is <see cref="ServiceLifetime.Transient" />.</param>
    /// <param name="configurationSectionKey">The configuration section key. Default is <see cref="DbContextSpark.ConfigurationSectionKey" />.</param>
    /// <remarks> This also registers the <see cref="DbContext"/> as a service in the services provided by the <paramref name="builder"/> with the same lifetime specified by <paramref name="lifetime"/>.</remarks>
