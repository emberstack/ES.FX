...
public sealed class ZendeskOrganizationTools(IZendeskClient zendeskApiClient)
{
    ...
    [McpServerTool(Name = "zendesk_organizations_list", ReadOnly = true, OpenWorld = true)]
    [Description(
        ...
        "exact name or external id use zendesk_organizations_search instead. Read-only.")]
    ...
    [McpServerTool(Name = "zendesk_organizations_count", ReadOnly = true, OpenWorld = true)]
    [Description(
    ...
    [McpServerTool(Name = "zendesk_organizations_read_many", ReadOnly = true, OpenWorld = true)]
    [Description(
        "Returns many Zendesk organizations by id in one call — the batch form of zendesk_organizations_read, ideal " +
    ...
    [McpServerTool(Name = "zendesk_organizations_search", ReadOnly = true, OpenWorld = true)]
    [Description(
        ...
        "zendesk_organizations_autocomplete. Read-only.")]
    ...
    [McpServerTool(Name = "zendesk_organizations_autocomplete", ReadOnly = true, OpenWorld = true)]
    [Description(
        "Suggests Zendesk organizations whose name starts with the given prefix — use when the exact name is " +
        "unknown (zendesk_organizations_search requires an exact match). Offset pagination only: " +
    ...
    [McpServerTool(Name = "zendesk_organizations_users", ReadOnly = true, OpenWorld = true)]
    [Description(
    ...
    [McpServerTool(Name = "zendesk_organizations_memberships", ReadOnly = true, OpenWorld = true)]
    [Description(
        ...
        "use zendesk_organizations_users instead. Offset pagination: 'count'/'next_page' indicate more pages. " +
        "Read-only.")]
    ...
    [McpServerTool(Name = "zendesk_organizations_merge_status", ReadOnly = true, OpenWorld = true)]
    [Description(
        "Returns the status of a Zendesk organization merge — poll after zendesk_organizations_merge until 'status' " +
        "is \"complete\" (or \"error\"). Note the id is the merge's own string id, not a job_status id — " +
        "zendesk_job_statuses_read does not track organization merges. Read-only.")]
    public Task<ZendeskOrganizationMerge> MergeStatus(
        [Description("The organization merge id (a string) returned by zendesk_organizations_merge.")]
        string mergeId,
        CancellationToken cancellationToken)
    ...
    [McpServerTool(Name = "zendesk_organizations_tags", ReadOnly = true, OpenWorld = true)]
    [Description(
        "Lists the tags set on a Zendesk organization (requires organization tagging to be enabled in Support). " +
        "Tags are changed via zendesk_organizations_update by sending the full replacement list. Read-only.")]
