...
public sealed class ZendeskJobStatusTools(IZendeskClient zendeskApiClient)
{
    ...
    [Description(
        "Lists recent Zendesk job statuses (the async jobs returned by bulk write tools), sorted first by " +
        "completion date, then by creation date, in descending order. Zendesk retains job status data for roughly " +
        "one day. Cursor pagination only: pass pageSize/afterCursor; the result's meta.has_more/meta.after_cursor " +
    ...
    public Task<ZendeskJobStatus> Read(
        [Description(
            "The job status id — an opaque string (not numeric), returned by the bulk operation that started the " +
            "job.")]
        string id,
        CancellationToken cancellationToken)
    ...
    public Task<ZendeskJobStatusesResult> ReadMany(
        [Description("The job status ids — opaque strings (not numeric); at most 100.")]
        string[] ids,
        CancellationToken cancellationToken)
