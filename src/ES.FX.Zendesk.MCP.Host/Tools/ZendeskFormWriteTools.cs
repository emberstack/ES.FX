...
public sealed class ZendeskFormWriteTools(IZendeskClient zendeskApiClient, IMcpExecutionModeAccessor executionMode)
{
    ...
    [Description(
        ...
        "(resolve field ids with ticket_fields_list). Returns the created ticket form. Write operation — " +
    ...
    [Description(
        ...
        "replaces the form's field list wholesale — read the current form with forms_get first and send " +
    ...
    [Description(
        ...
        "adjust it afterwards with forms_update. Write operation — honors the server execution mode: " +
        "rejected in read-only mode, simulated (no changes made) in dry-run mode.")]
