...
public interface IAnafClient
{
    ...
    Task<AnafCompany?> FindCompanyAsync(long cui, DateOnly? asOf = null,
    ...
    /// <param name="asOf">The reference date for the query; defaults to today (UTC).</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    Task<AnafCompanyBatch> FindCompaniesAsync(IReadOnlyCollection<long> cuis, DateOnly? asOf = null,
        CancellationToken cancellationToken = default);
}
